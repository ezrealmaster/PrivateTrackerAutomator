...
from utils import gib_to_bytes, mib_to_bytes


class TelegramListener:
    def __init__(self, config, session_name='anon'):
...
async def message_handler_hdolimpo(event):
    name, *properties, info = event.raw_text.splitlines()
    log.info(f"{name=}")
    log.info(f"{properties=}")
    # Download logic here...
    if "🎖🎖🎖 Premium 🎖🎖🎖" in properties:
    ...
    if "🔥🔥🔥 100% Free 🔥🔥🔥" in properties:
        ...
        log.info("Skipping non-free torrent.")
        return
    doubleup = False
    featured = False
    if "💎💎💎 Doble carga 💎💎💎" in properties:
    ...
    if "🚨🚨🚨 Sin Hit&Run 🚨🚨🚨" in properties:
        hrless = True
    if freeleech or featured:
        torrent_id = initialized_trackers[TrackerName.HDOLIMPO].search(name)
        if torrent_id is None:
            log.info("Skipping torrent, search returned no results.")
        log.info(f"Downloading torrent {torrent_id}...")
        try:
            match = re.search(r"(\d+(?:\.\d+)?)\s(GiB|MiB)", info)
            size = float(match.group(1))
...
async def message_handler_xbytesv2(event):
    url_pattern = r"https://xbytesv2\.li/torrents/(\d+)"
    try:
        torrent_id = re.search(url_pattern, event.text).group(1)
    except AttributeError as e:
        log.info("Ignoring message:")
    ...
    name, *stuff, discounts, info = event.raw_text.splitlines()
    log.info(f"{name=}")
    log.info(f"{discounts=}")
    # Download logic here...
    freeleech = int(re.match(r"Free: (\d+)%", discounts).group(1))

    doubleup = False
    featured = False
    hrless = False

    if freeleech == 100 or featured:
        log.info(f"Downloading torrent {torrent_id}...")
        try:
            match = re.search(r"(\d+(?:\.\d+)?)\s(GiB|MiB)", info)
            size = float(match.group(1))
            ...
            log.warning("Could not match size.")
            size = None
        torrent = TorrentInfo(name=name, tracker=TrackerName.XBYTESV2, size=size, id=torrent_id,
                              upload_date=datetime.now(),
                              freeleech=freeleech, doubleup=doubleup, featured=featured, hrless=hrless)
        downloadscheduler.new_torrent(torrent)


@events.register(events.NewMessage(chats=[-1001201573336]))
async def message_handler_divteam(event):
    url_pattern = r"https://divteam\.com/index\.php\?page=torrent-details(?:-ocio)?&id=([a-f0-9]+)"
    try:
        torrent_id = re.search(url_pattern, event.text).group(1)
    except AttributeError as e:
        log.info("Ignoring message:")
        log.info(event.raw_text)
        return
    log.debug(f"Matched {torrent_id=}")
    _, name, _, url = event.raw_text.splitlines()
    name = re.match(r"🎬 Título: (.+)", name).group(1)
    log.debug(f"Matched {name=}")
    # Download logic here...
    size, unit, freeleech = initialized_trackers[TrackerName.DIVTEAM].get_torrent_info(torrent_id)
    log.debug(f"Found size {size=}")
    log.debug(f"With unit {unit=}")
    log.debug(f"And discount {freeleech=}")
    ...
    if freeleech or featured:
        log.info(f"Downloading torrent {torrent_id}...")
        torrent = TorrentInfo(name=name, tracker=TrackerName.DIVTEAM, size=size, id=torrent_id,
                              upload_date=datetime.now(),
                              freeleech=freeleech, doubleup=doubleup, featured=featured, hrless=hrless)
        downloadscheduler.new_torrent(torrent)
...
async def message_handler_redbits(event):
    url_pattern = r"https://redbits\.xyz/torrents/(\d+)"
    try:
        torrent_id = re.search(url_pattern, event.text).group(1)
    except AttributeError as e:
        log.info("Ignoring message:")
    ...
    name, *stuff, info, _, url = event.raw_text.splitlines()
    log.info(f"{name=}")
    # Download logic here...
    freeleech = initialized_trackers[TrackerName.REDBITS].get_torrent_info(torrent_id)
    ...
    if freeleech == 100 or featured:
        ...
        try:
            ...
            log.warning("Could not match size.")
            size = None
        torrent = TorrentInfo(name=name, tracker=TrackerName.REDBITS, size=size, id=torrent_id,
                              upload_date=datetime.now(),
                              freeleech=freeleech, doubleup=doubleup, featured=featured, hrless=hrless)
        downloadscheduler.new_torrent(torrent)